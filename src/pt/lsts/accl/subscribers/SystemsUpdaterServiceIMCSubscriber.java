...
public class SystemsUpdaterServiceIMCSubscriber extends Service implements IMCSubscriber {
    ...
    private SystemList systemList = ACCL.getInstance().getSystemList();
    ...
    public void onReceive(final IMCMessage msg) {
        ...
        final ACCL.MODE mode = ACCL.getInstance().getMode();
        ...
        switch (mode){
            ...
            case SYSTEMLIST:
                ...
                break;
            case NONE:
                Log.v(TAG,"ACCL.mode==NONE, message ignored:\n"+msg.toString());
                break;
            default:
    ...
    public void processAnnounce(IMCMessage msg, SystemList systemList){
        ...
        systemList.changeList(systemList.getList());
        ACCL.getInstance().sysList = systemList;
    ...
    public void processEstimatedState(IMCMessage msg, Sys sys){
        ...
        if (altInt!=sys.getAltInt()){
            ...
            if (ACCL.getInstance().getActiveSys().equals(sys)){
                Log.i(TAG,"alt: getActiveSys().equals(sys)");
                ACCL.getInstance().getBus().post(new Pair<String,Integer>("alt",altInt));
            }
    ...
    public void processIndicatedSpeed(IMCMessage msg, Sys sys){
        ...
        if (iasInt!=sys.getIasInt()){
            ...
            if (ACCL.getInstance().getActiveSys().equals(sys)){
                ...
                ACCL.getInstance().getBus().post(new Pair<String,Integer>("ias",iasInt));
            }
        }
        //call OTTO
    }
    ...
    public void processPlanControlState(IMCMessage msg,Sys sys){
        if (IMCUtils.isMsgFromActive(msg)) {
            ...
            if (planControlState.getState() == PlanControlState.STATE.EXECUTING){
                ...
                String paintedPlanID = ACCL.getInstance().getActiveSys().getPaintedPlanID();
                boolean changed = ACCL.getInstance().getActiveSys().setPlanID(planID);
                if (!ACCL.getInstance().getActiveSys().isPaintedPlanUpdated()
    ...
    public int onStartCommand(Intent intent, int flags, int startId){
        ...
        ACCL.getInstance().addSubscriber(this);
    ...
    public void onDestroy() {
        Log.v(TAG,"onDestroy()");
        ACCL.getInstance().removeSubscriber(this);
        stopSelf();
    }
