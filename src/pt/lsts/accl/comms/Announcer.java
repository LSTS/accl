...
public class Announcer {
	...
	private Runnable task = new Runnable() {
		@Override
		public void run() {
			boolean announcePosition = ACCL.getInstance().getPrefs()
	...
	public void start() {
		gpsManager = ACCL.getInstance().getGpsManager();
		sensorManager = ACCL.getInstance().getSensorManager();
		...
		if (announcePosition)
			gpsManager.addListener(locListener);
		boolean announceheading = ACCL.getInstance().getPrefs()
	...
	private void updateLocationOnAnnounce() {
		...
		Location currentLocation = ACCL.getInstance().getGpsManager()
				.getCurrentLocation();
	...
	public void generateAnnounce() {
		...
		String sysName = "ACCL-01" + "_" + ip[2] + "." + ip[3];
		String sysType = "CCU";
		// double lat = currentLocation.getLatitude();
		// double lon = currentLocation.getLongitude();
		// double height = currentLocation.getAltitude();
