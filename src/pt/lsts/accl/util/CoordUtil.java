...
import pt.lsts.accl.pos.GISCoordinate;
import pt.lsts.accl.pos.UTMCoordinates;
import pt.lsts.imc.IMCMessage;

public class CoordUtil {
	/**
	 * Computes the offset (north, east) in meters from (lat, lon) to (alat,
	 * alon) [both of these in decimal degrees].<br>
	 * Subtract the two latlons and come up with the distance in meters N/S and
	 * E/W between them.
	 */
	public static double msToKnot = 1.9438612860586;

	public static double[] latLonDiff(double lat, double lon, double alat,
			double alon) {
	 ...
	 */
	public static double[] getAbsoluteLatLonDepth(double latitude,
			double longitude, double depth, double offsetNorth,
			double offsetEast, double offsetDown) {
	...
	/**
	 * Returns an Absolute Position based solely on a IMCMessage This message
	 * has to be an EstimatedState (or equivalent)
	 * 
	 * @param msg
	 *            Message to process
	...
	public static double[] getAbsoluteLatLonDepthFromMsg(IMCMessage msg) {
		...
		if (ref.equalsIgnoreCase("LLD_ONLY")) {
			...
			double[] lld = { msg.getDouble("lat"), msg.getDouble("lon"),
					msg.getDouble("depth") };
			return getAbsoluteLatLonDepth(Math.toDegrees(lld[0]),
					Math.toDegrees(lld[1]), Math.toDegrees(lld[2]), ned[0],
					ned[1], ned[2]);
		}
		if (ref.equalsIgnoreCase("NED_ONLY")) {
			double[] ned = { msg.getDouble("x"), msg.getDouble("y"),
					msg.getDouble("z") };
	 ...
	 * Modified by jqcorreia for ACCU
	 */
	public static double[] latLonAddNE2(double lat, double lon, double north,
			double east) {
		...
		try {
			...
			coord.move(dist * meterToFeet, Math.toDegrees(angRad),
					GISCoordinate.WGS84);

			double rest[] = latLonDiff(lat, lon, coord.getLatInDecDeg(),
					coord.getLonInDecDeg());
			rest[0] = 0;
			rest[1] = -rest[1] + east;
			...
			rest = latLonDiff(lat, lon, coord.getLatInDecDeg(),
					coord.getLonInDecDeg());
			rest[0] = -rest[0] + north;
			rest[1] = 0;
			angRad = Math.atan2(rest[1], rest[0]);
	 ...
	 */
	public static double dist2LatLon(double lat1, double lon1, double lat2,
			double lon2) {
		int R = 6371; // km
	 ...
	 */
	public static double bearing2LatLon(double lat1, double lon1, double lat2,
			double lon2) {
		double dLon = Math.toRadians(lon2 - lon1);
	 ...
	 *            Degrees to convert
	 * @param isLat
	 *            boolean to check if it is a latitude, if false the calculation
	 *            is done assuming longitude
