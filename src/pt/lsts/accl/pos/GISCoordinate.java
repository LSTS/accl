...
/**
 *
 * @author Tomer Petel (tomer@pacbell.net) Credits: Calculation routines
 *         (direct() and direct_ell()) are based on work done by Ed Williams.
 *         Ed's Code is at: http://williams.best.vwh.net/gccalc.htm
 */
public class GISCoordinate {
	...
	static {
		ells[0] = new Ellipsoid("Spherical (1'=1nm)", "Sphere",
				180 * 60 / Math.PI, Double.POSITIVE_INFINITY);
		ells[1] = new Ellipsoid("WGS84/NAD83/GRS80", "WGS84", 6378.137 / 1.852,
				298.257223563);
		ells[2] = new Ellipsoid("Clarke (1866)/NAD27", "NAD27",
				6378.2064 / 1.852, 294.9786982138);
		ells[3] = new Ellipsoid("International", "International",
				6378.388 / 1.852, 297.0);
		ells[4] = new Ellipsoid("Krasovsky", "Krasovsky", 6378.245 / 1.852,
				298.3);
		ells[5] = new Ellipsoid("Bessel (1841)", "Bessel", 6377.397155 / 1.852,
				299.1528);
		ells[6] = new Ellipsoid("WGS72", "WGS72", 6378.135 / 1.852, 298.26);
		ells[7] = new Ellipsoid("WGS66", "WGS66", 6378.145 / 1.852, 298.25);
		ells[8] = new Ellipsoid("FAI sphere", "FAI sphere", 6371.0 / 1.852,
				1000000000);
	...
	public void setCoordinates(String lat, String latDir, String lon,
			String lonDir) throws Exception {
	...
	public void printDEGMIN(java.io.PrintStream ps) {
		...
		String lons = degtodm(Math.abs(_lon));
		if (_printLonFirst) {
			ps.print(lons + "(" + (_lon < 0 ? "W" : "E") + ")" + _sep + lats
					+ "(" + (_lat < 0 ? "S" : "N") + ")");
		} else {
			ps.print(lats + "(" + (_lat < 0 ? "S" : "N") + ")" + _sep + lons
					+ "(" + (_lon < 0 ? "W" : "E") + ")");
		}
	}
	 ...
	 */
	public void move(double distft, double direction, int model)
			throws Exception {
		double distNM = distft / (185200.0 / 30.48); // convert to nm (does that
														// stand for nautical
														// miles??)
	...
	private void direct(double latRad, double lonRad, double dirRad,
			double distNM) throws Exception {
		double EPS = 0.00000000005;
		double dlon, lat, lon;
		if ((Math.abs(Math.cos(latRad)) < EPS)
				&& !(Math.abs(Math.sin(dirRad)) < EPS)) {
			throw new Exception(
					"Only N-S courses are meaningful, starting at a pole!");
		}

		lat = Math.asin(Math.sin(latRad) * Math.cos(distNM) + Math.cos(latRad)
				* Math.sin(distNM) * Math.cos(dirRad));
		if (Math.abs(Math.cos(lat)) < EPS) {
			lon = 0; // endpoint a pole
		} else {
			dlon = Math.atan2(
					Math.sin(dirRad) * Math.sin(distNM) * Math.cos(latRad),
					Math.cos(distNM) - Math.sin(latRad) * Math.sin(lat));
	...
	private void direct_ell(double glat1, double glon1, double faz, double s,
			Ellipsoid ellipse) throws Exception {
			...
			if (tok.hasMoreTokens()) {
				...
				if (tok.hasMoreTokens()) {
					...
					try {
						...
						throw new Exception(
								"minutes must be in decimal or integer format");
					}
					if (sec >= 60) {
	...
	private String degtodm(double degdec) {
		...
		return Integer.toString(deg) + ":" + Integer.toString(min) + ":"
				+ nf.format(sec);
	}
	...
	public static void main(String args[]) {
		try {
			GISCoordinate g = new GISCoordinate("37:55:15", "N", "122:20:59",
					"W");
			...
			double movedeg = 167;
			System.out.println("Moving " + movedist
					+ " Feet, in a direction of " + movedeg + " degrees...");
			...
			System.out.println("\ntesting the toString function:"
					+ GISCoordinate.FromString(g.toString()).toString());
		} catch (Throwable t) {
			t.printStackTrace();
		}
	}
