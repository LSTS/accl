 ...
 * to a series of listeners in a list. If there is no listener it auto-shutdown
 * the GPS locator
 * 
 * @author sharp
 *
...
public class GPSManager {
	...
	public void addListener(LocationChangeListener l) {
		// Verify if adding first listener to start GPS (battery expensive so
		// doesnt need to be up alll the time)
		if (listeners.size() == 0) {
			start();
		}
	...
	public void start() {
		if (!running) {
			...
			if (ACCL.getInstance().getPrefs().getBoolean("networkFix", false))
